// Compile with
//     g++ -std=c++17 polymorphism.cpp -o polymorphism
//
//
...
public:
  ...
  // method.
  //
  // It ends in ` = 0`, because in an interface (which is also the base class),
  // you don't specify any operations, you only say what are the methods are
  // that all subclasses *must* implement. To prevent the compiler from
...
public:
  ...
  // Time to override `advance`. I prefer to put the `virtual` at the beginning.
  // It's not mandatory, as long as you have either the virtual at the front or
...
int main() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    // using a smart poiter, this would be a memory leak. However, since we've
    ...
    // alone is enough to use a smart pointer.
  }
